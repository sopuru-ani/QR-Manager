...
function ChangePassword({ show, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${expressRoute}changepassword`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          currentPassword, // can be empty for Google users with password
          newPassword,
          confirmPassword,
        }),
      });
      ...
      if (response.ok) {
        setSuccessMsg(data.msg || "Password changed successfully!");
        setCurrentPassword("");
        setNewPassword("");
        setConfirmPassword("");
        setTimeout(() => {
    ...
    <div className="fixed inset-0 bg-gray-800/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <p className="text-gray-600 text-sm mb-4">
          Enter your current password (if any) and set a new password.
        </p>
